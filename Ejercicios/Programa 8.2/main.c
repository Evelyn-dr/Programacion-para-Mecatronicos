#include <stdio.h>
#include <stdlib.h> // Necesario para usar la funcion new

/* Estructuras-2.
El programa muestra la manera en que se declara una estructura, asi como la forma en que se tiene acceso a los campos de los apuntadores de tipo estructura tanto para lectura como para escritura. Se utiliza ademas una funcion que
recibe como para metro un apuntador de tipo estructura. */
...

{
    ...
    struct alumno a6; // Variable de tipo estructura alumno
    ...
    Para tener acceso a los campos de un apuntador de tipo estructura, utiliza uno de los dos formatos siguientes:
    apuntador->campo o bien (*apuntador).campo. En la lectura de los campos de la variable a4 se utilizan como ejemplo ambos formatos. */
    ...
    Lectura(&a6);  // En este caso se pasa la variable de tipo estructura alumno a6 a la funcion Lectura.
    printf("\nDatos del alumno 3\n");

    /* Observa la forma de escribir los campos de los apuntadores de tipo s
    estructura. */
    ...
    free(a4); // Liberamos la memoria asignada a a4
    free(a5); // Liberamos la memoria asignada a a5
}
