...
/* Estructuras-3.
El programa muestra la manera en que se declara una estructura anidada, asi como la forma de acceso a los campos de las variables o apuntadores de tipo estructura, tanto para lectura como para escritura. Se utiliza además una funcion que recibe como parámetro un apuntador de tipo estructura. */
...
/* Funcion que permite leer los campos de un apuntador de tipo estructura empleado. */
{
    ...
    fflush(stdin);
    printf("\tLocalidad: "); gets(a->direccion.localidad);
}
...
void main(void)
{
    ...
    printf("\tLocalidad: ");
    fflush(stdin);
    gets(e3.direccion.localidad); /* En la funcion Lectura se leen los campos de una variable, e4, y un apuntador de tipo estructura, *e2. */
    e2 = &e4;
    Lectura(e2);
    Lectura(&e4);
